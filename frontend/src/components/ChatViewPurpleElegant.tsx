...
export function ChatViewPurpleElegant() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[calc(100vh-220px)] font-serif">
      {/* Chat Panel */}
      <Card className="flex flex-col bg-white/90 backdrop-blur-sm shadow-lg border-purple-200">
        <div className="border-b border-purple-100 px-4 py-3 bg-gradient-to-r from-purple-50 to-indigo-100">
          <h2 className="text-purple-800 italic">Conversation</h2>
          <p className="text-sm text-purple-500">Chat to refine your search</p>
        </div>
          ...
          <div className="space-y-4">
              ...
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-lg px-4 py-2 ${
                  msg.role === 'user' 
                    ...
                    : 'bg-gradient-to-br from-violet-50 to-purple-100 text-purple-800 border border-purple-200'
                }`}>
                  <p>{msg.content}</p>
                  {msg.intent && (
                    <div className="mt-2 pt-2 border-t border-purple-200 space-y-1">
                      <p className="text-sm opacity-90 italic">Search Intent:</p>
                      <div className="flex flex-wrap gap-1">
                        ...
                        {msg.intent.brand && (
                          <Badge className="text-xs bg-indigo-100 text-indigo-800 hover:bg-indigo-200">
                            {msg.intent.brand}
                          </Badge>
                        )}
                        {msg.intent.model && (
            ...
            {loading && (
              <div className="flex justify-start">
                <div className="bg-gradient-to-br from-violet-50 to-purple-100 text-purple-800 rounded-lg px-4 py-2 border border-purple-200">
                  <p>Searching...</p>
                </div>
              </div>
            )}
      ...
      <Card className="flex flex-col bg-white/90 backdrop-blur-sm shadow-lg border-indigo-200">
        <div className="border-b border-indigo-100 px-4 py-3 bg-gradient-to-r from-indigo-50 to-purple-100">
          <h2 className="text-indigo-800 italic">Product Results</h2>
          <p className="text-sm text-indigo-500">{results ? `${results.items.length} products found` : 'No search yet'}</p>
        </div>
          ...
          <div className="space-y-4">
              ...
              <Card key={idx} className="p-4 hover:shadow-xl transition-all bg-gradient-to-br from-white to-purple-50 border-purple-100 hover:scale-[1.02]">
                {product.primary_image_url && (
                  <div className="mb-3 rounded-lg overflow-hidden bg-white">
                    <img 
                      src={getProxiedImageUrl(product.primary_image_url)} 
                      alt={product.normalized_title || product.title}
                ...
                <div className="flex justify-between items-start mb-3">
                  <div className="flex-1">
                    <h3 className="text-purple-800 mb-1 italic">{product.normalized_title || product.title}</h3>
                    <p className="text-sm text-purple-500">{product.source}</p>
                  </div>
                  {product.availability && (
                    <Badge className={product.availability.toLowerCase().includes('stock') ? "bg-violet-100 text-violet-800 hover:bg-violet-200" : "bg-pink-100 text-pink-800 hover:bg-pink-200"}>
                      {product.availability}
                    </Badge>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-purple-800">{formatPrice(product)}</p>
                      {product.is_listing_page && (
                        <p className="text-xs text-purple-500 mt-1">Category page - price range</p>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="text-purple-500">Match Score</span>
                      <span className="text-purple-800">{(product.match_score * 100).toFixed(0)}%</span>
                    </div>
                  ...
                  {product.review_sentiment !== 'unknown' && (
                    <div className="pt-2 border-t space-y-2">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-purple-500">
                            Sentiment: <span className="capitalize text-purple-800 italic">{product.review_sentiment}</span> {product.sentiment_score !== null && `(${(product.sentiment_score * 100).toFixed(0)}%)`}
                          </span>
                        </div>
                      ...
                      {product.sentiment_reason && (
                        <p className="text-xs text-purple-600 italic">{product.sentiment_reason}</p>
                      )}
                      
                      {product.sentiment_highlights && product.sentiment_highlights.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {product.sentiment_highlights.slice(0, 5).map((highlight, hIdx) => (
                            <Badge key={hIdx} variant="outline" className="text-xs bg-purple-50 text-purple-700 border-purple-200">
                              {highlight}
                            </Badge>
                          ))}
                        </div>
            ...
            {!results && (
              <div className="flex items-center justify-center h-full">
                <p className="text-purple-500 text-center italic">
